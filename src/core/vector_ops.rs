...
impl StreamingTopK {
    ...
    pub fn get_results(self) -> Vec<SearchResult> {
        let mut results: Vec<_> = self.heap.into_iter()
...
impl ProductQuantizer {
    ...
    pub fn train(&mut self, vectors: &[Vec<f32>], iterations: usize) {
        ...
        for subspace in 0..self.subspace_dim {
            ...
            let mut centroids = self.initialize_centroids_with_size(&subvectors, actual_num_centroids);
            
            for _ in 0..iterations {
                let assignments = self.assign_to_centroids(&subvectors, &centroids);
                centroids = self.update_centroids_with_size(&subvectors, &assignments, actual_num_centroids);
            }
    ...
    fn initialize_centroids_with_size(&self, vectors: &[Vec<f32>], num_centroids: usize) -> Vec<Vec<f32>> {
    ...
    fn update_centroids_with_size(&self, vectors: &[Vec<f32>], assignments: &[usize], num_centroids: usize) -> Vec<Vec<f32>> {
        if vectors.is_empty() {
            return vec![];
        }
