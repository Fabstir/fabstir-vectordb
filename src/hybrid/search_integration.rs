...
use crate::hybrid::core::{HybridIndex, HybridError, SearchConfig};
...
impl HybridIndex {
    pub async fn parallel_search(
        ...
        let timeout_result = tokio::time::timeout(config.timeout, async {
            ...
            let (recent_results, historical_results) = tokio::join!(
                recent_future,
                historical_future
            );
...
impl ResultMerger {
    ...
    pub fn merge(&self, result_sets: Vec<Vec<ScoredResult>>, k: usize) -> Vec<ScoredResult> {
        ...
        for (source_idx, results) in result_sets.iter().enumerate() {
            for result in results {
                merged_map.entry(result.vector_id.clone())
        ...
        for (vector_id, occurrences) in merged_map {
            let merged_result = match &self.strategy {
                ...
                MergeStrategy::Average => {
                    ...
                    let avg_distance: f32 = occurrences.iter().map(|(r, _)| r.distance).sum::<f32>() 
                ...
                if let Some(meta) = metadata {
                    if let Some(timestamp_str) = meta.get("timestamp") {
                        if let Ok(timestamp) = DateTime::parse_from_rfc3339(timestamp_str) {
                            ...
                            let decay_factor = 0.5_f32.powf(age.as_secs_f32() / half_life.as_secs_f32());
...
impl QueryOptimizer {
    ...
    pub async fn suggest_config(&self, _query: &[f32], k: usize) -> Result<SearchConfig, SearchIntegrationError> {
        let stats = self.index.get_statistics().await;
...
impl SearchPerformanceMonitor {
    ...
    pub async fn record_search(&self, latency: Duration, results_returned: usize, indices_searched: usize) {
    ...
    pub async fn get_statistics(&self) -> SearchStatistics {
        ...
        let mut latencies: Vec<f64> = stats.iter()
