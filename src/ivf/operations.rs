use crate::ivf::core::{IVFIndex, IVFConfig, IVFError, ClusterId, Centroid};
...
impl IVFIndex {
    ...
    pub fn batch_search(&self, queries: &[Vec<f32>], k: usize) -> Result<Vec<Vec<SearchResult>>, OperationError> {
    ...
    pub fn retrain(&mut self, new_config: IVFConfig) -> Result<RetrainResult, OperationError> {
        ...
        for i in 0..self.config.n_clusters {
            self.inverted_lists.insert(ClusterId(i), crate::ivf::core::InvertedList::new());
        }
    ...
    pub fn add_clusters(&mut self, n_clusters_to_add: usize) -> Result<AddClustersResult, OperationError> {
        if !self.is_trained() {
            return Err(IVFError::NotTrained.into());
        }
        
        if n_clusters_to_add == 0 {
            return Err(OperationError::InvalidParameter("Cannot add 0 clusters".to_string()));
        }
    ...
    pub fn estimate_memory_usage(&self) -> MemoryUsage {
        ...
        MemoryUsage {
            total_bytes: centroids_bytes + vectors_bytes + inverted_lists_bytes + structure_overhead,
            centroids_bytes,
            vectors_bytes,
    ...
    pub fn evaluate_search_quality(&self, test_queries: &[Vec<f32>], k: usize) -> Result<SearchQuality, OperationError> {
        if test_queries.is_empty() {
            return Err(OperationError::InvalidParameter("No test queries provided".to_string()));
        }
    ...
    pub fn balance_clusters(&mut self, threshold: f32) -> Result<BalanceResult, OperationError> {
        if threshold <= 0.0 || threshold >= 1.0 {
            return Err(OperationError::InvalidParameter("Threshold must be between 0 and 1".to_string()));
        }
        ...
        let mut cluster_sizes: Vec<(ClusterId, usize)> = self.inverted_lists
            .iter()
    ...
    pub fn import_centroids(&mut self, centroids_data: Vec<ExportedCentroid>) -> Result<(), OperationError> {
        if centroids_data.is_empty() {
            return Err(OperationError::InvalidParameter("No centroids to import".to_string()));
        }
        ...
        for centroid in &centroids_data {
            if centroid.dimension != dimension {
                return Err(OperationError::InvalidParameter("Inconsistent centroid dimensions".to_string()));
            }
        }
