...
mod hybrid_insertion_tests {
    ...
    async fn test_insert_recent_vector() {
        ...
        index.insert_with_timestamp(id.clone(), vector, Utc::now()).await.unwrap();
    ...
    async fn test_insert_historical_vector() {
        ...
        index.insert_with_timestamp(id.clone(), vector, old_timestamp).await.unwrap();
...
mod hybrid_search_tests {
    ...
    async fn test_search_recent_only() {
        ...
        println!("Results: {:?}", results.iter().map(|r| (r.vector_id.clone(), r.distance)).collect::<Vec<_>>());
    ...
    async fn test_search_historical_only() {
        ...
        // Debug counts
        println!("Recent count: {}, Historical count: {}", index.recent_count(), index.historical_count());
        
        let results = index.search(&vec![0.0, 2.5], 3).await.unwrap();
        
        assert_eq!(results.len(), 3);
        // Check that all results are from our historical vectors
        assert!(results.iter().all(|r| historical_ids.contains(&r.vector_id)));
    }
    ...
    async fn test_search_mixed() {
        ...
        let recent_results = results.iter()
            .filter(|r| recent_ids.contains(&r.vector_id))
            .count();
        let historical_results = results.iter()
    ...
    async fn test_search_with_config() {
        ...
        let results = index.search_with_config(&vec![0.5, 0.0], search_config).await.unwrap();
...
mod statistics_tests {
    ...
    async fn test_age_distribution() {
        ...
        for (i, timestamp) in timestamps.iter().enumerate() {
            let id = VectorId::from_string(&format!("vec_{}", i));
            let vector = vec![i as f32, 0.0];
            index.insert_with_timestamp(id, vector, *timestamp).await.unwrap();
        }
        ...
        assert_eq!(distribution.oldest_timestamp, now - chrono::Duration::days(30));
    }
}
