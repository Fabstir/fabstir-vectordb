...
mod hnsw_insertion_tests {
    ...
    fn test_insert_multiple_nodes() {
        ...
        for id in &ids {
            ...
            assert!(!neighbors.is_empty(), "Node should have neighbors at layer 0");
            assert!(neighbors.len() <= 8, "Should not exceed max_connections_layer_0");
        }
    }
