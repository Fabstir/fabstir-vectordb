    ...
    fn test_chunked_node_serialization() {
        ...
        for (chunk_id, chunk_data) in chunks {
            ...
            for (i, node) in deserialized.iter().enumerate() {
                ...
                assert_eq!(
                    node.id().to_string(),
                    VectorId::from_string(&expected_id).to_string()
                );
            }
        }
    }
...
mod hnsw_storage_tests {
    ...
    async fn test_save_and_load_large_index() {
        ...
        // Save
        persister
            .save_index(&index, "/test/large_hnsw")
            .await
            .unwrap();

        // Verify chunks were created
        let chunks = persister
            .storage()
    ...
    async fn test_incremental_save() {
        ...
        // Incremental save
        persister
            .save_incremental(&index, "/test/incremental", &dirty_nodes)
            .await
            .unwrap();
    ...
    async fn test_corrupted_data_handling() {
        ...
        storage
            .put("/bad/hnsw/metadata.cbor", vec![0xFF, 0xFF, 0xFF])
            .await
            .unwrap();
    ...
    async fn test_version_compatibility() {
        ...
        storage
            .put("/future/hnsw/metadata.cbor", metadata_bytes)
            .await
            .unwrap();
...
mod recovery_tests {
    ...
    async fn test_partial_save_recovery() {
        ...
        storage
            .put("/partial/hnsw/metadata.cbor", metadata.to_cbor().unwrap())
            .await
            .unwrap();
    ...
    async fn test_backup_and_restore() {
        ...
        // Save with backup
        persister
            .save_with_backup(&index, "/prod/hnsw", "/backup/hnsw")
            .await
            .unwrap();
        ...
        persister
            .storage()
            .delete("/prod/hnsw/metadata.cbor")
            .await
            .unwrap();
        ...
        // Restore from backup
        persister
            .restore_from_backup("/backup/hnsw", "/prod/hnsw")
            .await
            .unwrap();
