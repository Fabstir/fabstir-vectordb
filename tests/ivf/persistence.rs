...
mod ivf_storage_tests {
    ...
    async fn test_save_and_load_small_index() {
        ...
        for (name, vector) in &vectors {
            index
                .insert(VectorId::from_string(name), vector.clone())
                .unwrap();
        }
    ...
    async fn test_save_large_index_with_chunks() {
        ...
        // Save
        persister
            .save_index(&index, "/test/large_ivf")
            .await
            .unwrap();
        ...
        let files = persister
            .storage()
            .list("/test/large_ivf/inverted_lists/")
            .await
            .unwrap();
    ...
    async fn test_incremental_save() {
        ...
        persister
            .save_index(&index, "/test/incremental")
            .await
            .unwrap();
        ...
        // Incremental save
        persister
            .save_incremental(&index, "/test/incremental", &modified_clusters)
            .await
            .unwrap();
    ...
    async fn test_versioning() {
        ...
        storage
            .put(
                "/future/ivf/metadata.cbor",
                future_metadata.to_cbor().unwrap(),
            )
            .await
            .unwrap();
    ...
    async fn test_recovery_from_partial_save() {
        ...
        storage
            .put("/partial/ivf/metadata.cbor", metadata.to_cbor().unwrap())
            .await
            .unwrap();
        ...
        storage
            .put(
                "/partial/ivf/centroids.cbor",
                serialize_centroids(&centroids).unwrap(),
            )
            .await
            .unwrap();
    ...
    async fn test_compression_options() {
        ...
        persister_no_compress
            .save_index(&index, "/test/no_compress")
            .await
            .unwrap();
        ...
        persister_compress
            .save_index(&index, "/test/compress")
            .await
            .unwrap();

        // Compare sizes
        let files_no_compress = persister_no_compress
            .storage()
            .list("/test/no_compress/")
            .await
            .unwrap();
        let files_compress = persister_compress
            .storage()
            .list("/test/compress/")
            .await
            .unwrap();
        ...
        let size_no_compress =
            calculate_total_size(persister_no_compress.storage(), &files_no_compress).await;
        let size_compress =
            calculate_total_size(persister_compress.storage(), &files_compress).await;

        assert!(size_compress < size_no_compress);

        // Both should load correctly
        let loaded_no_compress = persister_no_compress
            .load_index("/test/no_compress")
            .await
            .unwrap();
        let loaded_compress = persister_compress
            .load_index("/test/compress")
            .await
            .unwrap();
...
mod ivf_migration_tests {
    ...
    async fn test_retrain_and_migrate() {
        ...
        // Migrate data
        let migration_result = persister
            .migrate_index("/test/v1", "/test/v2", new_config)
            .await
            .unwrap();
...
use vector_db::ivf::persistence::{calculate_total_size, serialize_centroids};
